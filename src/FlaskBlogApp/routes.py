...
def logout():
    ...
    flash("Έγινε αποσύνδεση του χρήστη")
    return redirect(url_for("root"))
...
def new_article():
    ...
    return render_template("new_article.html", form=form, page_title="Εισαγωγή Άρθρου")
...
def delete_article(article_id):
    ...
    flash("το άρθρο διεγράφη", "succes")
...
def account():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        flash(f"Ο λογαρισμός του χρήστη <b>{current_user.username}</b> ενημερώθηκε με επιτυχία", "succes")
